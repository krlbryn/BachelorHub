...
<head>
    ...
    <style>
        .election-header { margin-bottom: 30px; }
        .election-subtitle { font-family: 'Montserrat', sans-serif; font-size: 2em; color: #333; margin-top: 5px; }
        .position-section { margin-bottom: 40px; }
        .position-title { font-family: 'Montserrat', sans-serif; font-size: 1.8em; margin-bottom: 20px; color: #000; border-bottom: 2px solid #eee; padding-bottom: 10px; }
        .candidates-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; }
        .candidate-card { background: #fff; border-radius: 10px; padding: 20px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); text-align: center; display: flex; flex-direction: column; align-items: center; transition: transform 0.2s; }
        .candidate-card:hover { transform: translateY(-5px); }
        .candidate-img-container { width: 100px; height: 100px; border-radius: 50%; background-color: #3498db; display: flex; align-items: center; justify-content: center; margin-bottom: 15px; overflow: hidden; }
        .candidate-img-container i { font-size: 3em; color: #fff; }
        .candidate-img-container img { width: 100%; height: 100%; object-fit: cover; }
        .candidate-name { font-size: 1.2em; font-weight: bold; color: #000; margin-bottom: 5px; }
        .running-for { color: #555; font-size: 0.9em; margin-bottom: 15px; }
        .running-for span { color: #4481eb; font-weight: 500; }
        .candidate-details { text-align: left; width: 100%; font-size: 0.9em; color: #444; margin-bottom: 20px; background: #f9f9f9; padding: 10px; border-radius: 5px; min-height: 60px; }
        .btn-vote-candidate { background-color: #0d1b2a; color: white; border: none; padding: 10px 30px; border-radius: 25px; cursor: pointer; font-size: 1em; text-decoration: none; transition: background 0.3s; margin-top: auto; }
        .btn-vote-candidate:hover { background-color: #1b263b; }
        .modal-overlay { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); justify-content: center; align-items: center; z-index: 1000; }
        .vote-confirm-box { background: white; padding: 30px; border-radius: 10px; text-align: center; max-width: 400px; width: 90%; color: #333; }
        .vote-confirm-title { font-size: 1.5em; margin-bottom: 10px; color: #000; font-weight: bold; }
        .vote-confirm-text { margin-bottom: 25px; color: #666; font-size: 1.1em; }
        .vote-confirm-buttons { display: flex; justify-content: center; gap: 15px; }
        .btn-confirm-vote { background-color: #00b894; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 1em; }
        .btn-cancel-vote { background-color: #e74c3c; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 1em; }
    </style>
</head>
...
<body>
    ...
    <script>
        ...
        window.onclick = function(event) {
            if (event.target == document.getElementById('voteModal')) closeVoteModal();
